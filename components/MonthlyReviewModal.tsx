...
import { MonthlyAnalysisData } from '../types';
...
export const MonthlyReviewModal: React.FC<MonthlyReviewModalProps> = ({ isOpen, onClose, data: initialData }) => {
  ...
  const blurAmount = Math.min(20, (currentData.chaosLevel / 100) * 15);
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-3xl bg-white/90 backdrop-blur-2xl rounded-[48px] shadow-[0_32px_128px_-16px_rgba(0,0,0,0.2)] overflow-hidden flex flex-col max-h-[92vh] border border-white/60 animate-in fade-in zoom-in-95 duration-700">
        ...
        {/* Header - Crystal Clear Navigation */}
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400 font-bold uppercase tracking-[0.2em] mt-1">
                Generated at {format(new Date(currentData.timestamp), 'yyyy/MM/dd HH:mm')}
              </p>
            </div>
          </div>
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-4 custom-scrollbar">
                  {history.map((h) => (
                      <button
                          ...
                          className={`w-full flex items-center justify-between p-6 rounded-[32px] border transition-all ${currentData.id === h.id ? 'bg-slate-900 text-white border-slate-900 shadow-2xl scale-[1.02]' : 'bg-white text-slate-600 border-slate-100 hover:border-indigo-200 hover:bg-indigo-50/20'}`}
                      >
                          <div className="flex flex-col text-left">
                              <span className="text-base font-black">{format(new Date(h.timestamp), 'yyyy年MM月dd日')}</span>
                              <span className={`text-[11px] font-bold ${currentData.id === h.id ? 'opacity-60' : 'text-slate-400'}`}>{h.gradeTitle}</span>
                          </div>
                          <div className="flex items-center gap-6">
                              <span className={`text-3xl font-black ${currentData.id === h.id ? 'text-indigo-400' : getGradeColor(h.grade)}`}>{h.grade}</span>
                              {currentData.id === h.id && <Check size={20} className="text-indigo-400" />}
                          </div>
                      </button>
                  ))}
              </div>
        ...
        >
          {/* Hero Section: The Grade with Tech Visuals */}
          <div className="flex flex-col items-center justify-center py-12 min-h-[65vh] relative">
            ...
            <div className="relative flex items-center justify-center w-80 h-80">
              ...
              <div 
                ...
                style={{ 
                  transform: `scale(${1 + currentData.chaosLevel / 200})`,
              ...
              <div className="relative flex flex-col items-center animate-in zoom-in duration-1000 ease-out">
                <span className={`text-[140px] font-black ${getGradeColor(currentData.grade)} leading-none tracking-tighter drop-shadow-2xl`}>{currentData.grade}</span>
                <div className="mt-8 flex flex-col items-center gap-2">
                   ...
                   <span className="text-4xl font-black text-slate-800">{currentData.healthScore}</span>
                </div>
              </div>
            </div>
            
            <h3 className="mt-14 text-4xl font-black text-slate-800 text-center px-10 leading-[1.1] max-w-xl">{currentData.gradeTitle}</h3>
          ...
          <section className="space-y-10 pb-16">
             ...
             <div className="space-y-8">
                {currentData.candidAdvice.map((advice, idx) => (
